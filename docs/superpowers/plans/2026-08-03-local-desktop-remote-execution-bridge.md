...
**Interfaces:**
- Consumes: exact durable provisioning records from `JobRepository.get_provision_transaction(transaction_id)` and authenticated worker transaction reads.
...
def test_worker_artifact_ignores_generated_bytecode_cache(self):
    cache = self.repository_root / "remote_worker" / "__pycache__"
    cache.mkdir()
    (cache / "jobs.cpython-313.pyc").write_bytes(b"generated")
    result = build_worker_artifact(
        self.repository_root,
        self.temporary_root / "worker.tar.gz",
    )
    self.assertNotIn("remote_worker/__pycache__/jobs.cpython-313.pyc", result.members)
```
...
Expected: `ERROR` with `Worker source tree contains forbidden metadata.`
...
The fresh path must immediately start `apply_manifest`; a later polled `ready` record cannot replace the eventual failed/cancelled apply result.
...
git add cloud_run/session_service.py tests/python/test_session_service.py \
  tests/python/test_worker_server.py scripts/check.sh \
